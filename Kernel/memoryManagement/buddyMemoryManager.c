...
// // Enable this file by compiling with -DUSE_BUDDY_MM to avoid duplicate
// symbols #ifdef USE_BUDDY_MM
...
void *memory_alloc(uint64_t memoryToAllocate) {
  ...
  if (memoryToAllocate == 0 ||
      memoryToAllocate > memoryManager->size - memoryManager->used)
    return NULL;
  ...
  return (
      void *)(memoryManager->treeStart +
              (((uint64_t)node - getNodeLevel(exponent)) * POW_2(exponent)));
}

void memory_free(void *ptr) {
  if (memoryManager == 0 || ptr == NULL)
    return;
...
static int64_t getNodeIndex(uint8_t *ptr, uint8_t *exponent) {
  ...
  while (levelExponent > 0 && memoryManager->tree[node].state != OCCUPIED) {
    node = (int64_t)(((ptr - memoryManager->treeStart) >> levelExponent) +
                     getNodeLevel(levelExponent));
    levelExponent--;
  }
...
static int64_t findFreeNode(uint64_t node, uint8_t level, uint8_t targetLevel) {
  ...
  if (memoryManager->tree[node].state != SPLIT &&
      memoryManager->tree[node].state != FREE)
    return -1;
  int64_t left =
      findFreeNode(node * 2ULL + 1ULL, (uint8_t)(level + 1U), targetLevel);
  if (left != -1)
    return left;
...
static void setMerge(uint64_t node) {
  ...
  if (memoryManager->tree[buddy].state == FREE &&
      memoryManager->tree[node].state == FREE) {
