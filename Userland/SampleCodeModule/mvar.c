...
static int manager_main(int argc, char **argv) {
    ...
    for (int i = 0; i < writers && created < total; i++) {
        ...
        int64_t pid = my_create_process((entry_point_t)writer_main, wargv, "mvar_writer", NULL);
        if (pid < 0) {
            spawn_failed = 1;
            break;
        }
    ...
    for (int j = 0; j < readers && created < total && !spawn_failed; j++) {
        ...
        int64_t pid = my_create_process((entry_point_t)reader_main, rargv, "mvar_reader", NULL);
        if (pid < 0) {
            spawn_failed = 1;
            break;
        }
...
int mvar_command(int argc, char *argv[]) {
    if (argc != 2) {
        printf("Uso: mvar <writers> <readers>\n");
        return -1;
    }

    int writers = atoi(argv[0]);
    ...
    int64_t pid = my_create_process((entry_point_t)manager_main, manager_argv, "mvar_manager", NULL);
    if (pid < 0) {
